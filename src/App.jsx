...
2. フレーム1から10まで、1フレームずつ順番に投球結果を読み取る。数字の間違えやすい組み合わせ(例: 6と8、1と7、Xと数字)は特に注意して見る。フレームが進むごとに、今読んでいる数字が手順1で固定した行の高さから外れていないか都度確認する
3. 各フレームを読み終えたら、そのフレームの累計スコアが「前のフレームの累計 + このフレームで倒したピン数」と矛盾していないか自分で検算する。矛盾があれば、数字の読み取りを見直して修正する
4. 全フレームを読み終えたら、10フレーム目の累計スコアと、画面に表示されている「TOTAL」列の数字を突き合わせる。一致しない場合は、どこかのフレームの投球結果(特にストライク/スペアの見落とし、または他プレイヤーの行との混同)を読み間違えている可能性が高いので、frame_by_frame_readingを最初から見直し、一致するまで修正すること。TOTAL表示は画像上で最も読み取りやすい数字であることが多いため、最終的な正解の基準として扱う
